"""v5.5 labeler — adds photo_matches_category self-check field + relaxes
pintle chains-clipped semantics.

Changes vs v5.4 (small):
  1. Every describe response now includes a top-level
     `photo_matches_category` field (yes / no / unclear) — the model
     self-reports whether the image actually shows the hardware its source
     folder claims. Lets downstream training-data assembly filter out
     miscategorized images.
  2. `safety_chains_clipped_to_bar` enum relaxed from 5 values
     (both_clipped/one_clipped/neither_clipped/unclear/not_visible) to 4
     (at_least_one_clipped/none_clipped/unclear/not_visible). Matches
     uncle's actual inspection workflow on the side-angle photos that
     dominate the archive.

Reused unchanged from v5.4:
  - Multi-image few-shot prompt assembly (build_few_shot_request).
  - fifth_wheel + pintle_hook use v4's per-image localize-and-crop.
  - lock_jaws uses v5.2's deterministic fixed-band crop (rows 20–70%, full
    width) instead of per-image detection.
  - Few-shot library at training/assets/few_shot_examples/ (pintle caption
    updated to reflect the relaxed schema; library files unchanged).
"""
...
from trucksafe_training.labeling.prompts_v5_5 import (
    EVIDENCE_CONFIG,
    FEW_SHOT_SUBFOLDERS,
    FewShotExample,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="v5.5 labeler: photo_matches_category self-check + relaxed chains-clipped."
    )
