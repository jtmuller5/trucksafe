"""v5.4 labeler — few-shot multimodal prompts + simplified pintle schema.

Changes vs v5.2:
  1. Each describe() call prepends curated REFERENCE IMAGES with diagnostic
     captions in the same user-turn before the cropped INPUT image. The
     library lives at training/assets/few_shot_examples/ and is loaded once
     per run by prompts_v5_4.load_few_shot_library().
  2. Pintle (rear_assembly) schema has been simplified to the two-item
     inspection — hitch_latch_state, safety_chains_count,
     safety_chains_clipped_to_bar — see shared/schemas/pintle_hook*.json
     and prompts_v5_4.REAR_ASSEMBLY_PROMPT.

Reused unchanged from v5.2:
...
from trucksafe_training.labeling.prompts_v5_4 import (
    EVIDENCE_CONFIG,
    FEW_SHOT_SUBFOLDERS,
    FewShotExample,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="v5.4 labeler: few-shot multimodal + simplified pintle schema."
    )
