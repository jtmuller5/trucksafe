"""v5.7 labeler — fifth_wheel side_view crop padding.

Changes vs v5.6 (preprocessing only):
  - The detection bbox for fifth_wheel side_view is vertically padded by
    `vertical_padding_pct * bbox_height` on both the top and bottom (then
    clamped to image edges) before the crop is taken. The seating signal
    (`trailer_seated_flush`, `gap_between_apron_and_plate`) needs the
    apron-plate interface, not the fifth-wheel hardware in isolation, and
    v5.6 spot-check confirmed tight hardware-only crops were trimming off
    the apron context on at least 1-2 of the drifted side_view images.
  - Pintle and lock_jaws cropping is unchanged.

Unchanged from v5.6:
  - Schemas (side_view simplified, photo_matches_category top-level,
    relaxed pintle chains-clipped enum).
  - Prompts (prompts_v5_6 reused directly).
  - Few-shot library (training/assets/few_shot_examples/, side_view_plate).
  - Multi-image few-shot prompt assembly.
  - Per-image detection crop for pintle + fixed-band crop for lock_jaws.
  - Center-crop fallback when detection refuses (70% area).
"""
...
# v5.7 reuses prompts_v5_6 verbatim — no prompt change in this iteration.
...
# v5.7: side_view crops are vertically padded by this fraction of bbox height
# on both top and bottom (clamped to image edges), then cropped. Applies only
# to source_folder == "fifth_wheel". The default value is the first-guess
# from docs/labeling/v5-7.md; tune with --vertical-padding-pct.
SIDE_VIEW_VERTICAL_PADDING_PCT_DEFAULT = 0.30
SIDE_VIEW_PADDING_FOLDERS = {"fifth_wheel"}

LABELER_VERSION = "v5.7"
...
def apply_vertical_padding(
    ...
    """Expand [y1, x1, y2, x2] vertically by `pct * (y2 - y1)` on both top
    and bottom, clamped to image edges. Horizontal coords are unchanged."""
    y1, x1, y2, x2 = box_px
    bbox_h = max(0, y2 - y1)
    pad = int(round(bbox_h * pct))
    return [max(0, y1 - pad), x1, min(img_h, y2 + pad), x2]
...
def run_folder(
    ...
    vertical_padding_pct: float = SIDE_VIEW_VERTICAL_PADDING_PCT_DEFAULT,
) -> CategoryStats:
    ...
    if explicit_image_ids is not None:
        images = _resolve_images_from_ids(cat_dir, explicit_image_ids)
        print(f"  (using explicit image_ids — {len(images)} images)", flush=True)
    elif image_ids_from is not None:
    ...
    pad_this_folder = source_folder in SIDE_VIEW_PADDING_FOLDERS
    if pad_this_folder:
        print(f"  (fifth_wheel side_view: vertical padding +{vertical_padding_pct*100:.0f}% bbox height on top/bottom)", flush=True)
    ...
    for i, image_path in enumerate(images, 1):
        ...
        if do_detection_crop:
            ...
            if loc.status == "success":
                ...
                if pad_this_folder and vertical_padding_pct > 0:
                    crop_box = apply_vertical_padding(loc.expanded_box_pixels, img_w, img_h, vertical_padding_pct)
                else:
                    crop_box = loc.expanded_box_pixels
                crop_bytes = make_crop(image_path, crop_box)
            ...
            if loc.status == "success" and pad_this_folder and vertical_padding_pct > 0:
                loc_obj["vertical_padding_pct"] = vertical_padding_pct
                loc_obj["padded_crop_box_pixels"] = crop_box
            if loc.status != "success":
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="v5.7 labeler: vertical padding on fifth_wheel side_view detection crops."
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated image-id stems. Overrides random sampling AND --image-ids-from. Used for targeted probes.",
    )
    p.add_argument(
        "--vertical-padding-pct",
        type=float,
        default=SIDE_VIEW_VERTICAL_PADDING_PCT_DEFAULT,
        help=f"Vertical padding added to fifth_wheel side_view detection bbox (fraction of bbox height, both top and bottom, clamped to image). Default {SIDE_VIEW_VERTICAL_PADDING_PCT_DEFAULT}.",
    )
    args = p.parse_args(argv)

    explicit_image_ids: list[str] | None = None
    if args.image_ids:
        explicit_image_ids = [s.strip() for s in args.image_ids.split(",") if s.strip()]
        print(f"explicit image-ids ({len(explicit_image_ids)}): {explicit_image_ids}", flush=True)
    ...
    for needed in FEW_SHOT_SUBFOLDERS["fifth_wheel"]:
        if not few_shot_library.get(needed):
            raise SystemExit(
                f"v5.7 expects few-shot subfolder '{needed}' with at least one captioned image. "
    ...
    print(f"side_view vertical_padding_pct: {args.vertical_padding_pct}", flush=True)
    ...
    with httpx.Client() as client:
        for folder in folders:
            ...
            stats = run_folder(
                ...
                explicit_image_ids=explicit_image_ids,
                vertical_padding_pct=args.vertical_padding_pct,
            )
