#!/usr/bin/env python3
"""HTML reviewer for v5.6 labels (side_view schema simplification).

Adds on top of the v5.5 reviewer:
  1. Side_view cards no longer render manufacturer-conditional sub-objects
     (they were removed from the schema).
  2. Side_view cards include a v5.5 → v5.6 schema delta footnote.

Inherited from v5.5:
  - "Flagged miscategorizations" cohort at the top.
  - `photo_matches_category` badge on every card.
  - Pintle v5.4 → v5.5 + v5.2 → v5.4 schema delta footnotes.

Open `:8770/labels/audit-batch-08/review.html` to view over Tailscale.
"""
...
PMC_COLORS = {"yes": "#070", "no": "#c00", "unclear": "#c70"}

# Map source_folder → human evidence label + ordered list of few-shot
# subfolders embedded in the prompt for that folder. Mirrors
# prompts_v5_6.FEW_SHOT_SUBFOLDERS but stated locally so the reviewer
# is decoupled from the trucksafe_training package.
FEW_SHOT_SUBFOLDERS = {
    "fifth_wheel": ("side_view", ["side_view_plate"]),
    "lock_jaws": ("lock_jaws_underneath", ["lock_jaws"]),
    "pintle_hook": ("rear_assembly", ["pintle_hook"]),
}

SIDE_VIEW_SCHEMA_DELTA_HTML = (
    "<details class='delta'>"
    "<summary>v5.5 → v5.6 side_view schema delta</summary>"
    "<ul>"
    "<li><b>removed:</b> manufacturer-conditional sub-objects "
    "(<code>holland</code>, <code>fontaine</code>, <code>jost</code>) and their nested fields. "
    "v5.5 audit confirmed those signals were unreliable.</li>"
    "<li><b>demoted:</b> <code>fifth_wheel_manufacturer</code> is now descriptive only — "
    "no downstream conditional logic depends on it.</li>"
    "<li><b>retained primary signals:</b> <code>trailer_seated_flush</code>, "
    "<code>gap_between_apron_and_plate</code>.</li>"
    "</ul></details>"
)

PINTLE_SCHEMA_DELTA_V54_HTML = (
    "<details class='delta'>"
    "<summary>v5.4 → v5.5 pintle schema delta</summary>"
    "<ul>"
    "<li><b>relaxed enum:</b> <code>safety_chains_clipped_to_bar</code> from "
    "<code>both_clipped/one_clipped/neither_clipped/unclear/not_visible</code> → "
    "<code>at_least_one_clipped/none_clipped/unclear/not_visible</code> "
    "(matches uncle's actual side-angle inspection workflow).</li>"
    "</ul></details>"
)
...
def render_few_shot_section(
    ...
    return (
        ...
        "<h2>Few-shot reference images used by v5.6</h2>"
...
def render(
    ...
    def _pmc_of(rec: dict[str, Any]) -> str:
        return (rec.get("describe_output") or {}).get("photo_matches_category", "yes")

    flagged_records = [d for d in labels if _pmc_of(d) != "yes"]
    yes_records = [d for d in labels if _pmc_of(d) == "yes"]
    ordered = flagged_records + yes_records
    flagged_count = len(flagged_records)

    rows: list[str] = []
    flagged_section_header_inserted = False
    yes_section_header_inserted = False
    for idx, d in enumerate(ordered, 1):
        if not flagged_section_header_inserted and flagged_count > 0 and idx == 1:
            rows.append(
                f"<h2 class='cohort-header flagged-header'>"
                f"Flagged miscategorizations — {flagged_count} of {len(ordered)} "
                f"(<code>photo_matches_category != yes</code>)</h2>"
                "<p class='muted'>Spot-check these: do you agree with the model's call that the photo "
                "doesn't match its source-folder category? Tag each agree/disagree and note the totals "
                "back in <code>comparison_v5_4_v5_5.md</code>.</p>"
            )
            flagged_section_header_inserted = True
        if (
            flagged_count > 0
            and not yes_section_header_inserted
            and idx == flagged_count + 1
        ):
            rows.append(
                f"<h2 class='cohort-header yes-header'>"
                f"Photo matches category — remaining {len(yes_records)} images</h2>"
            )
            yes_section_header_inserted = True
        ...
        do = d.get("describe_output") or {}
        pmc_value = do.get("photo_matches_category", "<missing>")
        ...
        if sf == "pintle_hook":
            delta_html = PINTLE_SCHEMA_DELTA_V54_HTML + PINTLE_SCHEMA_DELTA_HTML
        elif sf == "fifth_wheel" and ev == "side_view":
            delta_html = SIDE_VIEW_SCHEMA_DELTA_HTML
        else:
            delta_html = ""
        pmc_color = PMC_COLORS.get(pmc_value, "#666")
        ...
        flagged_class = " flagged" if pmc_value != "yes" else ""

        rows.append(f"""
<section class="card{flagged_class}">
  <h3>[{idx}] {html.escape(sf)}/{html.escape(stem)} → <span class="pill">{html.escape(ev)}</span>
    <span class="badge pmc-badge" style="background:{pmc_color}">photo_matches: {html.escape(pmc_value)}</span>
  </h3>
  <div class="grid">
    <div class="col">
    ...
    <div class="col">
      <div class="col-label">v5.6 output</div>
      <p>
...
<title>v5.6 audit review — {html.escape(labels_dir.name)}</title>
<style>
  body {{ font-family: -apple-system, system-ui, sans-serif; margin: 0; padding: 18px; background: #fafafa; color: #222; }}
  h1 {{ margin: 0 0 20px; }}
  h2.cohort-header {{ margin: 24px 0 12px; font-size: 18px; padding: 8px 14px; border-radius: 6px; }}
  h2.flagged-header {{ background: #fde7e7; color: #851; border-left: 4px solid #c70; }}
  h2.yes-header {{ background: #eef5ec; color: #364; border-left: 4px solid #070; }}
  .card {{ background: white; padding: 14px; margin-bottom: 16px; border: 1px solid #ddd; border-radius: 8px; }}
  .card.flagged {{ border-color: #c70; background: #fffdf6; }}
  .pmc-badge {{ margin-left: 8px; font-size: 11px; }}
...
<h1>v5.6 audit review — {html.escape(labels_dir.name)} • {len(labels)} cards • {flagged_count} flagged</h1>
{refs_html}
{''.join(rows)}
</body></html>
"""
