#!/usr/bin/env python3
"""HTML reviewer for v5.5 labels (photo_matches_category self-check +
relaxed chains-clipped semantics).

Adds on top of the v5.4 reviewer:
  1. "Flagged miscategorizations" cohort at the top — every record where
     `photo_matches_category != yes`. The user reviews these and tags
     agree/disagree as the v5.5 spot-check step.
  2. `photo_matches_category` badge on every card.
  3. Pintle cards now show the v5.4 → v5.5 schema delta (chains-clipped
     enum relaxed from 5 values to 4) in addition to the v5.2 → v5.4 delta.

Open `:8770/labels/audit-batch-07/review.html` to view over Tailscale.
"""
...
def render(
    ...
    for idx, d in enumerate(ordered, 1):
        ...
        delta_html = (PINTLE_SCHEMA_DELTA_V54_HTML + PINTLE_SCHEMA_DELTA_HTML) if sf == "pintle_hook" else ""
...
<section class="card{flagged_class}">
  ...
  <div class="grid">
    ...
    <div class="col">
      <div class="col-label">v5.5 output</div>
      <p>
...
<title>v5.5 audit review — {html.escape(labels_dir.name)}</title>
<style>
...
<h1>v5.5 audit review — {html.escape(labels_dir.name)} • {len(labels)} cards • {flagged_count} flagged</h1>
{refs_html}
{''.join(rows)}
</body></html>
"""
